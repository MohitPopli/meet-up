...
describe('Dialog tests', () => {
  ...
  test('should render error dialog and perform validations', () => {
    render(<Dialog refObject={refObj} canScheduleMeeting={false} onConfirm={onConfirm} closeDialog={closeDialog} />);
    ...
    expect(screen.getByTestId(/error-content-dialog/i).querySelector('span')?.textContent).toEqual(
      'Sorry!! This time slot is currently unavailable for scheduling the meeting. Please select other time slot to schedule meeting.',
    );
  ...
  test('should render confirmation dialog and perform validations', () => {
    render(<Dialog refObject={refObj} canScheduleMeeting onConfirm={onConfirm} closeDialog={closeDialog} />);
    ...
    expect(screen.getByTestId(/confirmation-content-dialog/i).querySelector('label')?.textContent).toEqual(
      ' Reason for meeting ',
    );

    expect(
      screen
        .getByTestId(/confirmation-content-dialog/i)
  ...
  test('should render confirmation dialog and able to confirm meeting', () => {
    ...
    expect(screen.getByTestId(/meeting-dialog-confirm-custom-button/i).getAttribute('disabled')).toStrictEqual('');
    userEvent.type(
      screen.getByTestId(/confirmation-content-dialog/i).querySelector('textarea') as HTMLTextAreaElement,
      'confirm meeting',
    );
